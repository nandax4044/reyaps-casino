...
interface CaseOpeningGameProps {
  user: any;
  refreshUser: () => void;
}

export default function CaseOpeningGame({ user, refreshUser }: CaseOpeningGameProps) {
  ...
  // If chest selected but not found, reset selection
  useEffect(() => {
    if (selectedChestId && !activeChest && !loadingConfig) {
      console.warn('[CASE] Selected chest not found, resetting:', selectedChestId);
      setSelectedChestId(null);
    }
  }, [selectedChestId, activeChest, loadingConfig]);
  ...
  const startCaseUnboxing = () => {
    ...
    if (parseFloat(user.balance) < activeChest.price) {
      setBalanceError(`Saldo tidak mencukupi untuk membuka chest ini! Harga: ${activeChest.price.toLocaleString()} WL, Saldo Anda: ${parseFloat(user.balance).toLocaleString()} WL. silakan hubungi staff untuk deposit.`);
      stopAutoSpin();
      return;
    }
    setBalanceError('');
    ...
    const runReelSpin = () => {
      ...
      spinTimerRef.current = setTimeout(() => {
        ...
        API.addWinningItem({
          ...
          .then(() => {
            refreshUser(); // Refresh user details securely
          })
          .catch((err) => {
    ...
    // Deduct chest cost first
    API.deductBalance(activeChest.price)
      .then(() => {
        refreshUser();
        if (fastSkip) {
          runReelSpin();
      ...
      .catch((err) => {
        setBalanceError(err.message || 'Gagal memotong saldo bermain.');
        stopAutoSpin();
      });
  };
    ...
    <div className="w-full flex flex-col gap-6" id="case-opening-view">
      ...
      {loadingConfig ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-slate-400 text-sm font-mono">Memuat konfigurasi chest...</p>
        </div>
      ) : chests.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              >
                {reelItems.length === 0 ? (
                  ...
                  Array.from({length: 12}).map((_, idx) => {
                    ...
                    const rConf = RARITY_HIERARCHY[tempItem.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common;
                    return (
                      <div 
                        key={idx}
                  ...
                  reelItems.map((item, idx) => {
                    const rConf = RARITY_HIERARCHY[item.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common;
                    return (
                      <div 
                        key={idx}
            ...
            {showPrizeOverlay && winningItemResult && (
              <div className="bg-black/40 border border-white/10 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 animate-scale-up-sm relative overflow-hidden">
                <div 
                  ...
                  style={{ background: `radial-gradient(circle, ${(RARITY_HIERARCHY[winningItemResult.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common).color} 0%, transparent 70%)` }}
                />

                <div className="flex items-center gap-5 z-10 text-center sm:text-left flex-col sm:flex-row">
                  <div className="w-24 h-24 bg-black/60 rounded-2xl border-2 flex items-center justify-center shadow-2xl animate-bounce shrink-0" style={{ borderColor: (RARITY_HIERARCHY[winningItemResult.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common).color }}>
                    <ItemImage item={winningItemResult} className="w-16 h-16 object-contain" />
                  </div>
                  <div>
                    <div className="flex items-center justify-center sm:justify-start gap-1.5 flex-wrap">
                      <span className={`text-[10px] uppercase font-black px-2.5 py-0.5 rounded ${(RARITY_HIERARCHY[winningItemResult.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common).bg} ${(RARITY_HIERARCHY[winningItemResult.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common).textCol} border ${(RARITY_HIERARCHY[winningItemResult.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common).border} font-mono shadow-md`}>
                ...
                >
                  {isSpinning ? (
                    ...
                    <>
                      ...
                      <span>BUKA CHEST ({activeChest?.price?.toLocaleString()} WL)</span>
                    </>
                  )}
                </button>
          ...
          <div className="bg-[#141224]/30 border border-white/5 p-6 rounded-3xl flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {activeChest?.items.map((item, idx) => {
                const rh = RARITY_HIERARCHY[item.rarity as keyof typeof RARITY_HIERARCHY] || RARITY_HIERARCHY.Common;
                return (
                  <div 
                    key={idx}
