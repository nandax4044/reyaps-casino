...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {!isFullscreen && (
        <header className="border-b border-white/10 py-5 px-6 md:px-12 flex flex-col lg:flex-row justify-between items-center bg-[#070914]/85 backdrop-blur-md sticky top-0 z-40 gap-4">
          <div className="flex items-center gap-3 self-start lg:self-auto">
            <div className="w-11 h-11 bg-blue-500/10 rounded-2xl border border-blue-500/30 flex items-center justify-center p-1.5 bg-black/40 shadow-lg">
              <img src="/logo.png" alt="Logo" className="w-full h-full object-contain filter drop-shadow-[0_2px_4px_rgba(30,144,255,0.4)]" referrerPolicy="no-referrer" />
            </div>
            <div>
          ...
          {/* User info, Balance tracker and logout */}
          <div className="flex items-center gap-3.5 shrink-0 select-none">
            <div className="flex flex-col text-right">
              <span className="text-xs font-black text-slate-200 flex items-center justify-end gap-1">
                {user.username}
                {user.is_staff && (
                  <span className="text-[8px] tracking-wider uppercase font-mono font-black bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded border border-red-500/30">Admin</span>
                )}
              </span>
              <div className="flex items-center justify-end mt-1">
                <CurrencyDisplay balance={user.balance} size="xs" />
              </div>
            </div>
            ...
            <button
              ...
              className="p-2.5 bg-red-950/20 border border-red-500/10 hover:border-red-500/40 text-red-400 hover:bg-red-900/20 rounded-xl cursor-pointer transition-all hover:scale-105 active:scale-95"
              title="Keluar"
            >
